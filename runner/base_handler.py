...
class BaseHandler(object):
    ...
    def __init__(self, cfg):
        ...
        if not cfg['test']:
            ...
            if cfg['ckpt_for_eval'] == 'task_arithmetic':
                self.last_ckpt_path = osp.join(cfg['test_save_path'], 'model-last.pth')
                self.best_ckpt_path = osp.join(cfg['test_save_path'], 'model-best.pth')
                print("[setup] in test mode under task_arithmetic, so test_load_ckpt_path is null")
            else:
        ...
        # self.steplr is loaded in self.exec() after obtaining training set
    ...
    def func_load_model(cfg):
        ...
        elif 'init_ckpt' in cfg and cfg['init_ckpt']:
            path_source_model = osp.join(cfg['init_ckpt_path'], 'train_model-last.pth')
            source_model_ckpt = torch.load(path_source_model)
            model.load_state_dict(source_model_ckpt['model'], strict=True)
            print(f"[INFO] loaded network weights from {path_source_model}.")
        else:
            pass
    ...
    def func_load_lrs(optimizer, cfg, num_train_samples):
        ...
        if 'drop_last_batch' not in cfg or cfg['drop_last_batch'] is True:
            sgd_steps_in_one_epoch = len_dataloader // cfg['bp_every_batch']
        else:
            sgd_steps_in_one_epoch = int(math.ceil(len_dataloader / cfg['bp_every_batch']))
    ...
    def exec(self):
        ...
        # Here we load self.steplr as it relies on num_train_samples
        num_train_samples = len(train_set)
        print("[exec] # training samples:", num_train_samples)
        self.steplr = self.func_load_lrs(self.optimizer, self.cfg, num_train_samples)
        ...
        if 'force_to_skip_training' in self.cfg and self.cfg['force_to_skip_training']:
            ...
            val_name = 'validation'
            val_loaders = {'validation': val_loader}
            ...
            # true by default
            if 'eval_test_loader_per_epoch' in self.cfg and self.cfg['eval_test_loader_per_epoch'] is False:
                val_loaders['test'] = None
            else:
                val_loaders['test'] = test_loader
    ...
    def on_exec_test_start(self, test_set):
        pass
    ...
    def exec_test(self):
        ...
        test_splits = self.cfg['test_split'].split('-')
        
        def construct_eval_loader(split_name):
            # Prepare datasets 
            if split_name == 'train':
                ...
                pass
            print('[exec] test patient IDs from {}'.format(split_name))
            
            cur_dataset = self.func_prepare_dataset(pids, split_name, self.cfg, self.data_meta)
            self.uid.update({split_name: cur_dataset.uid})
    
            if split_name == 'test':
                self.on_exec_test_start(cur_dataset)
    
            # Evaluation on test_set
            cur_loader = DataLoader(cur_dataset, batch_size=self.cfg['batch_size'], 
                generator=seed_generator(self.cfg['seed']), num_workers=self.cfg['num_workers'], 
                shuffle=False, worker_init_fn=seed_worker, collate_fn=collate_func
            )
            return cur_loader
        
        evals_loader = {name: construct_eval_loader(name) for name in test_splits}
        ckpt_to_use  = self.cfg['ckpt_for_eval'] # best / last
        print("[exec] start evaluation on {} with the {} trained model.".format(test_splits, ckpt_to_use))
        remove_ckpt = self.cfg['remove_ckpt_after_test'] if 'remove_ckpt_after_test' in self.cfg else False
        metrics = self._eval_all(
            evals_loader, ckpt_type=ckpt_to_use, if_print=True, test_mode=True, 
            test_mode_name=mode_name, remove_ckpt_after_eval=remove_ckpt
        )
    ...
    def _eval_all(self, evals_loader, ckpt_type='best', run_name='train', task='bag_clf', if_print=True,
        ...
        elif ckpt_type == 'task_arithmetic':
            ckpt_path = add_prefix_to_filename(self.last_ckpt_path, 'taskari')
            wandb_group = 'lastckpt_ari/{}'.format(wandb_group_name)
            print_path = add_prefix_to_filename(self.last_metrics_path, metrics_path_name)
            csv_name = '{}_{}_last'.format(task, csv_prefix_name)
        else:
            pass
        ...
        for k, loader in evals_loader.items():
            ...
            if ckpt_type == 'task_arithmetic':
                cltor = self.test_model(self.net, loader, k, ckpt_path=None)
            else:
        ...
        if remove_ckpt_after_eval and osp.exists(ckpt_path):
            os.remove(ckpt_path)

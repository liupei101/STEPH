...
def load_cfg_src_ckpt(path_ckpt, datasets:str, coefficients:str, excluded_datasets=None):
    list_datasets = datasets.split('-')
    list_coefs = [float(c) for c in coefficients.split('-')]
    assert len(list_coefs) == len(list_datasets), "The number of datasets and coefficients do not match."

    if excluded_datasets:
        if isinstance(excluded_datasets, str):
            excluded_datasets = [excluded_datasets]
        new_list_datasets, new_list_coefs = [], []
        for i in range(len(list_datasets)):
            if list_datasets[i] in excluded_datasets:
                continue
            new_list_datasets.append(list_datasets[i])
            new_list_coefs.append(list_coefs[i])
        list_datasets, list_coefs = new_list_datasets, new_list_coefs

    res = dict()
    for i in range(len(list_datasets)):
        dataset = list_datasets[i]
        res[dataset] = {
            'path_ckpt': path_ckpt.format(dataset),
            'init_coef': list_coefs[i]
        }
    return res
...
def sampling_data(data, num:Union[int,float], seed=42):
    ...
    if isinstance(data, Tensor):
        return data[idxs_sampled], data[idxs_left]
    else:

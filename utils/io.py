...
def load_config_from_yaml(config_path, **kwargs):
    with open(config_path, "r") as setting:
        config = yaml.load(setting, Loader=yaml.FullLoader)
    # override the `key` field in config
    for k, v in kwargs.items():
        if k in config and v is not None:
            if isinstance(config[k], list) or config[k] is None:
                config[k] = [v]
            else:
                config[k] = v
            print(f"[INFO] `{k}` is overrided and set to {config[k]}")
